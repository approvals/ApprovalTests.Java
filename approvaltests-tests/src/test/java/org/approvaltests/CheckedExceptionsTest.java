...
public class CheckedExceptionsTest
{
  ...
  private Queryable<Class<?>> getAllClasses() {
    ...
    Queryable<String> paths = Query.select(files, Functions.unchecked(f -> f.getCanonicalPath().replace(File.separatorChar, '.')));
    paths = paths.where(p -> p.contains("main")).where(p -> p.contains(".approvaltests.src.") || p.contains(".approvaltests-util."));
